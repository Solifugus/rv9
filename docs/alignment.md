...
| 3. RT admission | **done for what can be checked** — `fork_rt` refuses before allocating: self-contradictory declaration, no slot free, stack+heap not guaranteeable, the 70% utilisation ceiling, a device this machine lacks, or a device somebody else owns. The last two apply to ordinary processes too. Real-time priority is now derived (R9 §13): admission places the whole workload across the two host priorities there are — above the radio, and below it — by response-time analysis, moves running loops when that changes, and refuses a set that fits the CPU but not the deadlines (design.md §33) |
| 4. protected reserves | **done** — a 12 KB floor RV-9 will not allocate into, so exhaustion is a refusal it reports rather than an abort inside ESP-IDF; above it an 8 KB reserve ordinary work cannot touch, so a control loop can still be admitted and a failsafe still applied after ordinary memory is gone; and per-process budgets, charged to a process and its ancestors, so a program that forks without end stops at its own budget. When `kill` or `procs` cannot be started, the shell does their jobs itself (design.md §35). What long-running use slowly consumed is bounded too: exited processes are forgotten beyond a history of 16/32, pids are reused safely across the wrap, and the network daemons' stacks are sized from measurement, which took idle heap from 29 to 44 KB (design.md §32) |
...
| 7. RT-safe marked machine-readably | **done** — `docs/target/rv9-profile.json` is generated from the sources by `tools/mkprofile.py` and marks every call, file manager and driver from the `RV9_RT_CODE` attribute on the function implementing it (`yes`, `device`, `no`); a stale profile fails the host tests. The board's own half — devices, limits, where the radio sits — comes from the `profile` command (design.md §34) |
...
| 12. extensible manifest | **done** — `rv9_mod_header_t.manifest_offset` points at a TLV list; seventeen tags registered (`on_deadline` the latest), `build.conf` emits them, `tools/modinfo.py` reads them back. The full registry — numbers, encodings, repetition, value names, and which tags anything actually enforces — is in the generated target profile |
| 13. resource certificate | **begun** — admission checks the declaration against itself (deadline within period, WCET within deadline) and against the machine. `control` declares 50 us and reports 26-30 us observed, which is a claim RV-9 can check rather than believe. Declared WCET now carries weight: it is the C in the response-time analysis that places loops and refuses unschedulable ones, and `rt` shows each loop's analysed bound beside its worst measured response |
