...
| 4. protected reserves | **partial** — a 12 KB floor RV-9 will not allocate into, so exhaustion is a refusal it reports rather than an abort inside ESP-IDF; no per-process limit, no RT-specific reserve. What long-running use slowly consumed is now bounded instead: exited processes are forgotten beyond a history of 16/32, pids are reused safely across the wrap, and the network daemons' stacks are sized from measurement, which took idle heap from 29 to 44 KB (design.md §32) |
