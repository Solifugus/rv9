 ...
 * kernel is running.
 *
 * Allocation comes from the kernel's own heap. The only thing still
 * borrowed is the region that heap sits in, which a kernel that owns the
 * machine would take from its boot information.
 */
...
/* The kernel's own heap. The contract's memory tests now exercise the
   allocator RV-9 wrote, not the one it used to borrow. */
static void *n_alloc(size_t n) { return rv9k_alloc(n); }
...
{
    rv9k_sem_t *s = (rv9k_sem_t *)rv9k_alloc(sizeof(*s));
    if (s == NULL) return -1;
    rv9k_sem_init(s, (int32_t)initial, (int32_t)max);
    *out = s;
    return 0;
}
static void n_sem_destroy(void *s) { rv9k_free(s); }
...
{
    rv9k_mutex_t *m = (rv9k_mutex_t *)rv9k_alloc(sizeof(*m));
    if (m == NULL) return -1;
    rv9k_mutex_init(m);
    *out = m;
    return 0;
...
static void n_mutex_destroy(void *m) { rv9k_free(m); }
...
{
    native_queue_t *nq =
        (native_queue_t *)rv9k_alloc(sizeof(*nq) + (size_t)len * item);
    if (nq == NULL) return -1;
...
static void n_queue_destroy(void *q) { rv9k_free(q); }
...
static void *n_calloc(size_t c, size_t s) { return rv9k_calloc(c, s); }
static void  n_free(void *p)              { rv9k_free(p); }
